...
sdlJeu::sdlJeu () : jeu() {
	...
	im_lavePleine.loadFromFile("data/lavePleine.png",renderer);
	...
	im_fondVictoire.loadFromFile("data/fondVictoire.png",renderer);
	...
	im_inventaire.loadFromFile("data/inventaire.png", renderer);
  	im_dragonDroite.loadFromFile("data/dragonDroite.png", renderer);
	im_dragonGauche.loadFromFile("data/dragonGauche.png", renderer);
  	...
  	im_plateformeMobile.loadFromFile("data/plateformeMobile.png", renderer);
    	im_personnageGaucheInvincible.loadFromFile("data/personnageInvincibleGauche.png", renderer);
    	im_personnageDroiteInvincible.loadFromFile("data/personnageInvincibleDroite.png", renderer);
    	im_personnageAttaqueDroiteInvincible.loadFromFile("data/personnageInvincibleAttaqueDroite.png", renderer);
    	im_personnageAttaqueGaucheInvincible.loadFromFile("data/personnageInvincibleAttaqueGauche.png", renderer);
	im_plateformeLave.loadFromFile("data/plateformeLave.png",renderer);
	im_bouton.loadFromFile("data/bouton.png",renderer);
	im_vieDragon.loadFromFile("data/vieDragon.png",renderer);
	im_barreVie.loadFromFile("data/barreVie.png",renderer);
...
void sdlJeu::sdlAff (unsigned int cote, unsigned int & attaque, unsigned int & testMort, unsigned int & testFondS1, unsigned int invulnerabilite, unsigned int test) {
	...
	if (testMort > 0) {
		...
		else if (salle == 19) im_fondVictoire.draw(renderer,0,0,1280,480);
		...
		// Afficher les sprites des murs
        	for (x=40*salle;x<40*(salle+1);++x) {
				...
				if (ter.getXY(x,y)=='@') im_plateformeLave.draw(renderer,(x-40*salle)*TAILLE_SPRITE,y*TAILLE_SPRITE,TAILLE_SPRITE,TAILLE_SPRITE);
				if (ter.getXY(x,y)=='?') im_mur.draw(renderer,(x-40*salle)*TAILLE_SPRITE,y*TAILLE_SPRITE,TAILLE_SPRITE,TAILLE_SPRITE);
				if (ter.getXY(x,y)=='!') im_lavePleine.draw(renderer,(x-40*salle)*TAILLE_SPRITE,y*TAILLE_SPRITE,TAILLE_SPRITE,TAILLE_SPRITE);
				if (ter.getXY(x,y)==')') im_bouton.draw(renderer,(x-40*salle)*TAILLE_SPRITE,y*TAILLE_SPRITE,TAILLE_SPRITE,TAILLE_SPRITE);
        		}
        	}

		// Affichage des plateformes
		for (unsigned int k = 0; k < NB_PLATEFORME; k++) {
			for (int l = 0; l < jeu.getPlateforme(k).getLongueur(); l++) {
				im_plateformeMobile.draw(renderer,(jeu.getPlateforme(k).getX()+l-40*salle)*TAILLE_SPRITE,jeu.getPlateforme(k).getY()*TAILLE_SPRITE,TAILLE_SPRITE,TAILLE_SPRITE);
			}
		}
		...
		if (bonus > 22) im_superJump.draw(renderer,(perso.getX()-40*salle)*TAILLE_SPRITE,(perso.getY()-1.5)*TAILLE_SPRITE,TAILLE_SPRITE,TAILLE_SPRITE);
		else if (bonus > 11) im_jambon.draw(renderer,(perso.getX()-40*salle)*TAILLE_SPRITE,(perso.getY()-1.5)*TAILLE_SPRITE,TAILLE_SPRITE,TAILLE_SPRITE);
		else if (bonus > 1) im_couteauDroite.draw(renderer,(perso.getX()-40*salle)*TAILLE_SPRITE,(perso.getY()-1.5)*TAILLE_SPRITE,TAILLE_SPRITE,TAILLE_SPRITE);
        	...
        	if (attaque == 0) {
        		if (cote == 2) {
                		...
                		if (invulnerabilite != 0) im_personnageDroiteInvincible.draw(renderer,(perso.getX()-40*salle)*TAILLE_SPRITE,(perso.getY()-0.5)*TAILLE_SPRITE,TAILLE_SPRITE*1.5,TAILLE_SPRITE*1.5);
              		}
        		else if (cote == 1) {
             			im_personnageGauche.draw(renderer,((perso.getX()-0.5)-40*salle)*TAILLE_SPRITE,(perso.getY()-0.5)*TAILLE_SPRITE,TAILLE_SPRITE*1.5,TAILLE_SPRITE*1.5);
              			if (invulnerabilite != 0) im_personnageGaucheInvincible.draw(renderer,((perso.getX()-0.5)-40*salle)*TAILLE_SPRITE,(perso.getY()-0.5)*TAILLE_SPRITE,TAILLE_SPRITE*1.5,TAILLE_SPRITE*1.5);
        	  	}
          	}
        		...
        		if (cote == 1) {
              			...
              			if (invulnerabilite != 0) im_personnageAttaqueGaucheInvincible.draw(renderer,((perso.getX()-0.5)-40*salle)*TAILLE_SPRITE,(perso.getY()-0.5)*TAILLE_SPRITE,TAILLE_SPRITE*1.5,TAILLE_SPRITE*1.5);
            		}
        		else if (cote == 2) {
              			...
              			if (invulnerabilite != 0) im_personnageAttaqueDroiteInvincible.draw(renderer,(perso.getX()-40*salle)*TAILLE_SPRITE,(perso.getY()-0.5)*TAILLE_SPRITE,TAILLE_SPRITE*1.5,TAILLE_SPRITE*1.5);
            		}		
        	}
		...
		for (unsigned int k=0; k<NB_PROJECTILE_DRAGON;k++) {
			if (jeu.getProjectileDragon(k).getX() >=0) {  // utile si les projectiles sont morts
				if (jeu.getProjectileDragon(k).getSalle()-1 == salle) {
					if (jeu.getProjectileDragon(k).getType() == 2) im_bouleDeFeu.draw(renderer,(jeu.getProjectileDragon(k).getX()-40*salle)*TAILLE_SPRITE,(jeu.getProjectileDragon(k).getY())*TAILLE_SPRITE,TAILLE_SPRITE,TAILLE_SPRITE);
				}
			}
		}
		...
		if (test%2 == 1) { // Quand les ennemis se deplacent
					...
					if (jeu.getEnnemis(k).getSalle()-1 == salle) {
						if (jeu.getEnnemis(k).getType() == 1) {  // zombie
							...
							if (jeu.getEnnemis(k).getDir() == 0) {
								...
								if (jeu.getEnnemis(k).getBouclierCouteau() >0) im_bouclierGauche.draw(renderer,(jeu.getEnnemis(k).getX()-1.5-40*salle)*TAILLE_SPRITE,(jeu.getEnnemis(k).getY()-0.5)*TAILLE_SPRITE,TAILLE_SPRITE*1.5,TAILLE_SPRITE*1.5);
			   				 }
			    				else {
			      					...
			      					if (jeu.getEnnemis(k).getBouclierCouteau() >0) im_bouclierDroite.draw(renderer,(jeu.getEnnemis(k).getX()+0.5-40*salle)*TAILLE_SPRITE,(jeu.getEnnemis(k).getY()-0.5)*TAILLE_SPRITE,TAILLE_SPRITE*1.5,TAILLE_SPRITE*1.5);
							}
						}
					}
				}
		...
		else { // Quand les ennemis ne se deplacent pas (affichage entre deux)
			for (unsigned int k=0; k<NB_ENNEMIS;k++) {
				if (jeu.getEnnemis(k).getX() >=0) { // utile si les ennemis sont morts
					if (jeu.getEnnemis(k).getSalle()-1 == salle) {
						if (jeu.getEnnemis(k).getType() == 1) {  // zombie
							if (jeu.getEnnemis(k).getDir() == 0) im_zombieGauche.draw(renderer,(jeu.getEnnemis(k).getX()-1-40*salle)*TAILLE_SPRITE,(jeu.getEnnemis(k).getY()-0.5)*TAILLE_SPRITE,TAILLE_SPRITE*1.5,TAILLE_SPRITE*1.5);
							else im_zombieDroite.draw(renderer,(jeu.getEnnemis(k).getX()-40*salle)*TAILLE_SPRITE,(jeu.getEnnemis(k).getY()-0.5)*TAILLE_SPRITE,TAILLE_SPRITE*1.5,TAILLE_SPRITE*1.5);
						}
						else if (jeu.getEnnemis(k).getType() == 2) {  // chauveSouris
							im_chauveSouris.draw(renderer,(jeu.getEnnemis(k).getX()-0.5-40*salle)*TAILLE_SPRITE,(jeu.getEnnemis(k).getY()-0.5)*TAILLE_SPRITE,TAILLE_SPRITE*1.5,TAILLE_SPRITE*1.5);
						}
						else if (jeu.getEnnemis(k).getType() == 3) {  // slime
							if (jeu.getEnnemis(k).getDir() == 0) { im_slimeGauche.draw(renderer,(jeu.getEnnemis(k).getX()-1-40*salle)*TAILLE_SPRITE,(jeu.getEnnemis(k).getY()-0.5)*TAILLE_SPRITE,TAILLE_SPRITE*1.5,TAILLE_SPRITE*1.5);}
							else im_slimeDroite.draw(renderer,(jeu.getEnnemis(k).getX()-40*salle)*TAILLE_SPRITE,(jeu.getEnnemis(k).getY()-0.5)*TAILLE_SPRITE,TAILLE_SPRITE*1.5,TAILLE_SPRITE*1.5);
						}
						else if (jeu.getEnnemis(k).getType() == 4) {  // petit_slime
							if (jeu.getEnnemis(k).getDir() == 0) { im_petitSlimeGauche.draw(renderer,(jeu.getEnnemis(k).getX()-0.5-40*salle)*TAILLE_SPRITE,jeu.getEnnemis(k).getY()*TAILLE_SPRITE,TAILLE_SPRITE,TAILLE_SPRITE);}
							else im_petitSlimeDroite.draw(renderer,(jeu.getEnnemis(k).getX()+0.5-40*salle)*TAILLE_SPRITE,jeu.getEnnemis(k).getY()*TAILLE_SPRITE,TAILLE_SPRITE,TAILLE_SPRITE);
						}
						else if (jeu.getEnnemis(k).getType() == 5) {  // sorcier
							if (jeu.getEnnemis(k).getDir() == 0) {
								im_sorcierGauche.draw(renderer,(jeu.getEnnemis(k).getX()-0.5-40*salle)*TAILLE_SPRITE,(jeu.getEnnemis(k).getY()-0.5)*TAILLE_SPRITE,TAILLE_SPRITE*1.5,TAILLE_SPRITE*1.5);
		...
		if (perso.getX() <= jeu.getDragon().getX()+3 && jeu.getDragon().getDirX() == -1) im_dragonGauche.draw(renderer,(jeu.getDragon().getX()-40*salle)*TAILLE_SPRITE,(jeu.getDragon().getY()-3)*TAILLE_SPRITE,TAILLE_SPRITE*6,TAILLE_SPRITE*4);
		else im_dragonDroite.draw(renderer,(jeu.getDragon().getX()-40*salle)*TAILLE_SPRITE,(jeu.getDragon().getY()-3)*TAILLE_SPRITE,TAILLE_SPRITE*6,TAILLE_SPRITE*4);


		// Affichage de la vie du dragon
        	if (salle > 15) {
			if (jeu.getDragon().getVieTotale() > 0) im_barreVie.draw(renderer,20.9*TAILLE_SPRITE,TAILLE_SPRITE,10.2*TAILLE_SPRITE,TAILLE_SPRITE);
        		for (i=1;i<=jeu.getDragon().getVieTotale();i++) {
        			im_vieDragon.draw(renderer,(20+i)*TAILLE_SPRITE,TAILLE_SPRITE,TAILLE_SPRITE,TAILLE_SPRITE);
        		}
        	}
		...
		if (salle > 0 && salle != 19) {
			...
			positionScore.x = 15*TAILLE_SPRITE;
      			positionScore.y = 2*TAILLE_SPRITE;
			positionScore.w = 5*TAILLE_SPRITE;
			positionScore.h = 2*TAILLE_SPRITE;
			textureScore = SDL_CreateTextureFromSurface(renderer,texteScore);
			SDL_FreeSurface(texteScore);
       			SDL_UpdateTexture(textureScore,NULL,texteScore->pixels,texteScore->pitch);
			SDL_RenderCopy(renderer,textureScore,NULL,&positionScore);
...
void sdlJeu:: sdlBoucle () {
	...
	unsigned int test=0, invulnerabilite=0, jump=0, cote=2, attaque=0, testLance = 0, testMort = 0, testFondS1 = 0;
	int nbProj = 0, nbProjEnnemis = 0, nbProjDragon = 0, testRepoussage = 0;
	...
	while (quit) {
		...
		if (nt-t>75) {
			...
			if ((ter.numeroSalle(perso.getX(),perso.getY()) > 1) && (testMort == 0) && testRepoussage == 0) {
				...
				if (jump > 4) jeu.actionClavier(' ',cote,nbProj,testRepoussage);
			...
			if (testRepoussage != 0) {
				if (testRepoussage > 0) {
					if (ter.estPositionPersoValideRetour(perso.getX()-1, perso.getY())) perso.setX(perso.getX()-1);
			  	      	if (not(ter.estPositionStructure(perso.getX(),perso.getY()+1))) perso.setY(perso.getY()+1);
				  	testRepoussage--;
				}
			      	if (testRepoussage < 0) {
				  if (ter.estPositionPersoValideRetour(perso.getX()+1, perso.getY())) perso.setX(perso.getX()+1);
			...
			if (testMort == 0) jeu.actionsAutomatiques(test,invulnerabilite,jump, perso.getX(), perso.getY(), nbProjEnnemis,  nbProjDragon, testMort);

		}
		sdlAff(cote,attaque,testMort,testFondS1, invulnerabilite, test);
