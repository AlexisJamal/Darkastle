...
void Projectile::mouvementProjectile (Terrain & t) {
  	int xtmp;
	xtmp = x + dir;
	if ((t.estPositionProjectileValide(xtmp,y)) && (t.numeroSalle(xtmp,y)==salle)) x = xtmp;
    	else mortProjectile();
}

void Projectile::mouvementProjectileDragon (Terrain & t,int dragonX, int dragonY) {
    	int xtmp, ytmp;

	if (dragonX-basePersoX >= 0) {
		if ((dragonX - basePersoX) <= 5) {
			ytmp = y + 1;
			if (y != basePersoY) {
				if ((t.estPositionProjectileValide(x,ytmp)) && (t.numeroSalle(x,ytmp)==salle)) y = ytmp;
				else mortProjectile();
			}
			else {
				xtmp = x - 1;
