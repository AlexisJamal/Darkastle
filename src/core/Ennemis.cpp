...
int Ennemis::getBouclierCouteau () const { return bouclierCouteau; }

void Ennemis::setBouclierCouteau (const int nvBouclierCouteau) { bouclierCouteau = nvBouclierCouteau; }
...
void Ennemis::gravite (const Terrain & t) {
...
void Ennemis::volAuto (Terrain & t, const int PersoX, const int PersoY) {
			...
			if (y>PersoY) {
				...
				if (dir == 0) xtmp = x-1;
				if (dir == 1) xtmp = x+1;
				if (t.estPositionEnnemisValideVolant(xtmp,y)) x = xtmp;
		   		else if (dir == 0) dir = 1;
				else if (dir == 1) dir = 0;
			}
			else {
